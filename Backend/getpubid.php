...
$limit = isset($_GET['limit']) ? max(1, min(100, (int)$_GET['limit'])) : 2; // Limitez à 100 pour éviter les abus
$start = ($page - 1) * $limit;

// Définir le fuseau horaire de MySQL à UTC+0
mysqli_query($con, "SET time_zone = '+00:00'");

// Requête SQL sécurisée avec une requête préparée pour récupérer une publication spécifique avec pagination et countdown
$sql = "
SELECT p.*, 
       ...
       IF(p.date = 'non' OR p.datefin = 'non', 'non', 
          IF(NOW() < STR_TO_DATE(p.date, '%Y-%m-%dT%H:%i:%s') 
             ...
             IF(NOW() < STR_TO_DATE(p.datefin, '%Y-%m-%dT%H:%i:%s') 
                AND STR_TO_DATE(p.date, '%Y-%m-%dT%H:%i:%s') < STR_TO_DATE(p.datefin, '%Y-%m-%dT%H:%i:%s'),
                'Événement en cours!',
                IF(STR_TO_DATE(p.date, '%Y-%m-%dT%H:%i:%s') = STR_TO_DATE(p.datefin, '%Y-%m-%dT%H:%i:%s'),
                   'non',
                   'Événement terminé'
                )
             )
