...
function generateUniqueID($con) {
    do {
        $id = uniqid();
        $query = mysqli_query($con, "SELECT * FROM `etatdelikes_resto` WHERE `id` = '$id'");
...
$sql = "INSERT INTO etatdelikes_resto (id, id_resto, iduser, etat) VALUES (?, ?, ?, ?)";
$stmt = mysqli_prepare($con, $sql);

// Vérifier si la préparation de la requête a réussi
if ($stmt) {
    // Binder les paramètres à la requête préparée
    mysqli_stmt_bind_param($stmt, "ssss", $id, $id_resto, $iduser, $etat);
