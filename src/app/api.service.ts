...
headers : HttpHeaders;
  ...
  adr_mobile: string='http://192.168.1.72:80';
...
loadcommentairepub(id){
  return this.http.get(this.base_url+'/Projet_ Lokaliser/Backend/getcomment.php?id='+id)
  .pipe(timeout(15000))
  .pipe(retry(1), catchError(this.handleError))
}

loadMoreComments(pubId: string, page: number): Observable<any> {
  return this.http.get(`${this.base_url}/Projet_ Lokaliser/Backend/loadMoreComments.php?id=${pubId}&page=${page}`)
    .pipe(timeout(15000), retry(1), catchError(this.handleError));
}
...
getpmu(){
  ...
  loadsignalement(){
    return this.http.get(this.base_url+'/Projet_ Lokaliser/Backend/loadsignalement.php')
    .pipe(timeout(15000))
    .pipe(retry(0), catchError(this.handleError));
    }
